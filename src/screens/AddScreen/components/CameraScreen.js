  ...
  const applyFilters = async (videoUris, lastVideo) => {
    ...
    if (!selectedSound && lastVideo.currentSpeed === 1) {
      ...
      await RNFFmpeg.execute(
        `-ss ${lastVideo.startAudio} -t ${lastVideo.endAudio} -i '${lastVideo.uri}' -ss ${lastVideo.startAudio} -t ${lastVideo.endAudio} -itsoffset ${audioDelay + 0.2} -i '${selectedSound.url}' -c copy -map 0:v:0 -map 1:a:0 -shortest -q 1 ${processedVideoFile}`
      );
